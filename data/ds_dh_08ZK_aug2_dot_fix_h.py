...
def extract_data3(file_path, cfg):
    ...
    try:
        if np.isnan(y_data_series[-1]):
            is_histogram = True
#             print(file_path)
#             y_data_series = y_data_series[:-1]
        else:
            is_histogram = False
    except:
        is_histogram = False
    ...
    x_data_series_3 = ';'.join(map(str, x_data_series))
    y_data_series_3 = ';'.join(map(str, y_data_series))
    x_data_series_4 = ';'.join(map(str, x_data_series[::-1]))
    y_data_series_4 = ';'.join(map(str, y_data_series[::-1]))
...
self.df.query('source_type == "extracted"').chart_type.value_counts()
vertical_bar      5392 -> 6370
line              1741 -> 2383
horizontal_bar    1405 -> 1619
scatter            894 -> 894
...
hist_ids_xy = [item + '_x' for item in hist_ids] + [item + '_y' for item in hist_ids]
...
class CustomDataset(Dataset):
    def __init__(self, df, cfg, aug, mode="train"):
        ...
        self.df.loc[self.df.fold==7, 'image_path'] = self.df.loc[self.df.fold==7, 'image_path'].str.replace(cfg.data_folder, cfg.hist_folder)
        self.df.loc[self.df.fold==7, 'anno_path'] = self.df.loc[self.df.fold==7, 'anno_path'].str.replace(cfg.data_folder, cfg.hist_folder)
...
#         print(gtdf[gtdf['id'].isin(hist_ids_xy)])
        ...
        if mode == "train":
            ...
            dfpm2['image_path'] = cfg.data_dir + '/pubmed/' + dfpm2['filename']
            ...
            '''
            # Unlabelled pseudo
            annos = json.load(open(cfg.pubmed_anno2))
            annos = annos['_via_img_metadata']
            annos = {k:v for k,v in annos.items() if v['file_attributes']['quality']=='good'}
            adfls = []
            for k,v in annos.items():
                vout = copy.deepcopy(v)
                fname = v["filename"].split('/')[-1]
                if len(vout['regions'])>0:
                    bbox = vout['regions'][0]['shape_attributes']
                    x0,y0,x1,y1 = bbox['x'], bbox['y'], bbox['x']+bbox['width'], bbox['y']+bbox['height']
                    img = cv2.imread( f'{cfg.pubmed_data_dir2}/{fname}' )
                    imgout = img[x0:y1,x0:x1]
                    fname = 'crop__' + fname
                    cv2.imwrite(f'{cfg.pubmed_data_dir2}/{fname}', imgout)
                vout['image_path'] = f'{cfg.pubmed_data_dir2}/{fname}'
                vout['text_series'] = vout['file_attributes']['text_series']
                vout['chart_type'] = vout['file_attributes']['chart_type'].replace('h_bar', 'horizontal_bar').replace('v_bar', 'vertical_bar').replace('histogram', 'vertical_bar')
                vout['source_type'] = 'extracted'
                
                series = [[j.strip() for j in i.split(';')] for i in vout['text_series'].split('|')]
                xseq, yseq = [i[0] for i in series], [i[1] for i in series]
                xseq, yseq = f'{";".join(xseq)}~~{";".join(xseq[::-1])}', f'{";".join(yseq)}~~{";".join(yseq[::-1])}'
                vout ['flattened_label'] = f"{xseq}||{vout['chart_type']}||numerical||{yseq}||{vout['chart_type']}||numerical"
                del vout['regions'], vout['size'], vout['file_attributes'], vout['filename']
                adfls .append( vout )
            adf = pd.DataFrame(adfls)          
            self.df = pd.concat([adf, self.df]).reset_index(drop = True)  
            ...
            self.df['image_key'] = self.df.image_path.str.split('/').str[-1]
            # Drop duplicates on image name, and keep the last which would be the manually labelled
            self.df = self.df.drop_duplicates(subset = 'image_key', keep = 'last')
            
            print(pd.crosstab(self.df.chart_type, self.df.source_type))
            
            '''
            source_type     extracted  generated
            chart_type                          
            dot                     0       1108
            horizontal_bar       1449          0
            line                 2224       5223
            scatter               894       2373
            vertical_bar         5773       3785
            '''
